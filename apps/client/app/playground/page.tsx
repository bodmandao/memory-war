    ...
    <motion.div initial="initial" animate="animate" transition={{ staggerChildren: 0.08 }} className="space-y-8">
      <motion.div variants={{ initial: { opacity: 0, y: 8 }, animate: { opacity: 1, y: 0 } }} transition={{ duration: 0.3 }}>
        ...
        <p className="mt-2 max-w-2xl text-[13px] leading-relaxed text-ink-dim">
          Four controlled runs of the actual protocol against {networkName ?? "the chain"} — not a mock. Each one drives
          real transactions and shows its own real, unedited step trace below as it happens, not a fixed animation. A
          full run typically moves through {REFERENCE_STAGES.join(" → ")} — but the exact steps shown are whatever this
          run actually did.
        </p>
      </motion.div>
